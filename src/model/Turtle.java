...
public class Turtle {
	 ...
	 * 
	 * @param ids
	 *            the indices of turtles to be active
	 ...
	 * 
	 * @param ids
	 *            are the indicies of turtles to activate
	 */
	...
	private class TurtleActioner {
		...
		private void paint(Graphics2D pen) {
			...
			if (highlight) {
				...
				pen.drawRect((int) myCenter.getX() - WIDTH_OF_TURTLE_ICON / 2,
						...
						WIDTH_OF_TURTLE_ICON, HEIGHT_OF_TURTLE_ICON);
			}
		...
		private void makeLine() {
			if (myPenStatus) {
				myLines.add(new StraightLine(myLastCenter, myCenter, myColors
						.get(myPenColor)));
			}
		}
